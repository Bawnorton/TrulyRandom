...
abstract class ChunkGeneratorMixin {
    ...
    *///?} else {
    ...
    private StructureStart randomiseStructurePlacement(Structure instance, Holder<Structure> selected, ResourceKey<Level> dimension, RegistryAccess registryAccess, ChunkGenerator chunkGenerator, BiomeSource biomeSource, Climate.Sampler climateSampler, RandomState randomState, StructureTemplateManager structureTemplateManager, long seed, ChunkPos sourceChunkPos, int references, LevelHeightAccessor heightAccessor, Predicate<Holder<Biome>> validBiome, Operation<StructureStart> original) {
        WorldGenHolder worldGenHolder = RandomiserSaveLoader.getWorldGenHolder();
        Modules modules = worldGenHolder.modules();
        StructureModuleState state = modules.getState(Module.STRUCTURES, StructureModuleState.class);
        if(state.isEnabled()) {
            if (state.useLegacyRandomiser()) {
                return original.call(instance, selected, dimension, registryAccess, chunkGenerator, biomeSource, climateSampler, randomState, structureTemplateManager, seed, sourceChunkPos, references, heightAccessor, Predicates.alwaysTrue());
            } else {
                if (instance.type().equals(StructureType.STRONGHOLD)) {
                    return original.call(instance, selected, dimension, registryAccess, chunkGenerator, biomeSource, climateSampler, randomState, structureTemplateManager, seed, sourceChunkPos, references, heightAccessor, validBiome);
                }
                ...
                StructureStart start = original.call(structure, selected, dimension, registryAccess, chunkGenerator, biomeSource, climateSampler, randomState, structureTemplateManager, seed, sourceChunkPos, references, heightAccessor, Predicates.alwaysTrue());
                randomiser.registerReplacement(dimension, sourceChunkPos, instance, structure);
                for (StructurePiece piece : start.getPieces()) {
                    BoundingBox boundingBox = piece.getBoundingBox();
                    ChunkPos minXZ = new ChunkPos(
